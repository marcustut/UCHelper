...
public class EventHandler {
    ...
    public static String getAttendeeList(Event e) {
        ...
        for (int index = 1; index <= e.getAttendeeListSize(); ++index) {
            result += String.format("%-5d %-15s %-15s %-15d", index, e.getAttendeeList(index),
                    e.getAttendeePosition(index), e.getAttendeeCocuMarks(index)) + "\n";
        }
        return result;
    }
    ...
    public static void displayEventListWithAttendeeList() {
        ...
        for (Iterator<Event> it = eventList.iterator(); it.hasNext();) {
            ...
            String toDisplay = String.format("%-16s%-15s \n%-16s%-15s \n%-16s%-15s \n%-40s\n%-40s\n%-40s\n",
                    "ID            : ", e.getEventID(), "Title         : ", e.getEventTitle(), "Organisor     : ",
                    e.getOrganisor(), "Attendee List : ", attendeeHeading(), getAttendeeList(e));
